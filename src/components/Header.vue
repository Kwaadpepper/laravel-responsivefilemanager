                    ...
                    <div class="row-fluid">
                        ...
                        <div class="span6 entire types">
                            ...
                            @if ($type_param != 1 && $type_param != 3 && config('rfm.show_filter_buttons'))
                                @switch(true)
                                    @case(count(config('rfm.ext_file')) > 0)
                                        ...
                                        @break
                                    @case(count(config('rfm.ext_img')) > 0)
                                        ...
                                        @break
                                    @case(count(config('rfm.ext_misc')) > 0)
                                        ...
                                        @break
                                    @case(count(config('rfm.ext_video')) > 0)
                                        ...
                                        @break
                                    @case(count(config('rfm.ext_music')) > 0)
                                ...
                                @endswitch
                            @endif
                            <input accesskey="f" type="text" class="filter-input @if ($type_param != 1 && $type_param != 3) filter-input-notype @endif" id="filter-input" name="filter" placeholder="{{ RFM::fixStrtolower(__('Text_filter')) }}..." value="{{ $filter }}" />
                            @if ($n_files > config('rfm.file_number_limit_js'))
                                <label id="filter" class="btn"><i class="icon-play"></i></label>
                            @endif
                            <input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide" />
                            <label id="ff-item-type-all" title="{{ __('All') }}" @if ($type_param == 1 || $type_param == 3) style="visibility: hidden;" @endif data-item="ff-item-type-all" for="select-type-all" style="margin-right:0px;" class="tip btn btn-inverse ff-label-type-all">{{ __('All') }}</label>
                        </div>
                    </div>
                </div>
            </div>
